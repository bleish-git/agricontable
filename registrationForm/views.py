...
def sign_up(request):
    if request.method == "POST":
        ...
        fm = SignUpForm()
    return render(request, 'registrationForm/signup.html', {'form':fm})
...
def user_login(request):
    if not request.user.is_authenticated:
        ...
        return render(request, 'registrationForm/userlogin.html', {'form': fm})
    else:
        return HttpResponseRedirect('/profile/')
    ...
    if request.user.is_authenticated:
        if request.method == "POST":
            ...
            if request.user.is_superuser == True:
                ...
                users = None
        return render(request, 'registrationForm/profile.html', 
...
def user_change_password(request):
    if request.user.is_authenticated:
        ...
        return render(request, 'registrationForm/changePassword.html', {'form': fm})
    else:
        return HttpResponseRedirect('/login/')
...
def user_change_password2(request):
    if request.user.is_authenticated:
        ...
        return render(request, 'registrationForm/changePassword2.html', {'form': fm})
    else:
        return HttpResponseRedirect('/login/')
...
def user_detail(request, id):
    if request.user.is_authenticated:
        ...
        return render(request, 'registrationForm/userdetail.html', 
...
def user_dashboard(request, id):
    if request.user.is_authenticated:
        pi = User.objects.get(pk=id)
        fm = EditAdminProfileForm(instance=pi)
        return render(request, 'registrationForm/userdashboard.html', 
            {'name': request.user.username,'form': fm})
    else:
        return HttpResponseRedirect('/login/')
